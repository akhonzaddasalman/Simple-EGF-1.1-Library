(() =>
{
    ...
    // Bookshelf DOM references

    const bookshelfView = $("bookshelfView");
    const gamePlayer = $("gamePlayer");
    const bookshelfGrid = $("bookshelfGrid");
    const bookshelfEmpty = $("bookshelfEmpty");
    const bookshelfSubtitle = $("bookshelfSubtitle");
    const btnAddGame = $("btnAddGame");
    const btnAddFirstGame = $("btnAddFirstGame");
    const btnBackToLibrary = $("btnBackToLibrary");
    const btnSaveToLibrary = $("btnSaveToLibrary");
    const bookContextMenu = $("bookContextMenu");
    const ctxPlay = $("ctxPlay");
    const ctxRemove = $("ctxRemove");

    // Game controls in header (hidden on bookshelf view)
    ...
    let currentLoadedFileData = null;
    ...
    function showBookshelf()
    {
        ...
        // Hide game controls in header
        gameControls.forEach(el => el.style.display = "none");
    }

    function showGamePlayer()
    {
    ...
    async function renderBookshelf()
    {
        try
        {
            libraryGames = await getAllGames();
        }
        catch (e)
        {
        ...
        libraryGames.sort((a, b) => (b.addedAt || 0) - (a.addedAt || 0));

        // Update subtitle
        const count = libraryGames.length;
        bookshelfSubtitle.textContent = count === 0
            ? t("bookshelfEmpty") || "Your EGF games will appear here"
            : (t("bookshelfCount") || "{count} games in your library").replace("{count}", count);
        ...
        if (count === 0)
        {
            // Hide the "Add Game" button when library is empty
            if (btnAddGame) btnAddGame.style.display = "none";

            // Show empty state with single button
            const emptyDiv = document.createElement("div");
            emptyDiv.className = "bookshelf-empty";
            emptyDiv.id = "bookshelfEmpty";
            emptyDiv.innerHTML = `
                <div class="empty-icon">📖</div>
                <h3>${t("noGamesYet") || "No games yet"}</h3>
                <p>${t("addFirstGame") || "Add your first EGF game to start playing!"}</p>
                <button id="btnAddFirstGame" type="button" class="btn">
                    ➕ ${t("addFirstGameBtn") || "Add Your First Game"}
                </button>
            `;
            bookshelfGrid.appendChild(emptyDiv);

            // Re-attach event listener
            document.getElementById("btnAddFirstGame")?.addEventListener("click", triggerFileInput);
        }
        else
        {
            // Show the "Add Game" button when library has games
            if (btnAddGame) btnAddGame.style.display = "";

            // Render book cards
            for (const game of libraryGames)
            {
                const card = createBookCard(game);
                bookshelfGrid.appendChild(card);
            }
        }
    }

    function createBookCard(game)
    {
        ...
        {
            coverHtml = `<img class="book-cover" src="${game.coverDataUrl}" alt="${escapeHtml(game.title || "Game cover")}" />`;
        }
        else
        {
        ...
        card.innerHTML = `
            ${coverHtml}
            <div class="book-info">
                <div class="book-title">${escapeHtml(game.title || "Untitled Game")}</div>
                <div class="book-creator">${escapeHtml(game.creator || "Unknown creator")}</div>
            </div>
            <button class="book-menu-btn" title="Options">⋮</button>
        `;
        ...
        card.addEventListener("click", (e) =>
        {
            ...
            playGameFromLibrary(game.id);
        });

        // Context menu button
        const menuBtn = card.querySelector(".book-menu-btn");
        menuBtn.addEventListener("click", (e) =>
        {
            e.stopPropagation();
            showContextMenu(e, game.id);
        });
    ...
    function showContextMenu(e, gameId)
    {
        ...
        bookContextMenu.style.display = "block";
        ...
        const menuRect = bookContextMenu.getBoundingClientRect();
        const maxX = window.innerWidth - 170;
        const maxY = window.innerHeight - 100;

        bookContextMenu.style.left = Math.min(x, maxX) + "px";
        bookContextMenu.style.top = Math.min(y, maxY) + "px";
    }

    function hideContextMenu()
    {
        bookContextMenu.style.display = "none";
        currentContextGameId = null;
    }

    async function playGameFromLibrary(gameId)
    {
        try
        {
            const game = await getGame(gameId);

            if (!game || !game.fileData)
            {
                alert(t("gameNotFound") || "Game not found in library.");
                return;
            }

            currentLoadedGameId = gameId;
            currentLoadedFileData = game.fileData;

            // Create a blob from the stored ArrayBuffer
            const blob = new Blob([game.fileData], { type: "application/octet-stream" });
            const file = new File([blob], game.fileName || "game.egf", { type: "application/octet-stream" });
        ...
        catch (e)
        {
            ...
            alert(t("errorPlayingGame") || "Error loading game from library.");
        }
    }

    async function removeGameFromLibrary(gameId)
    {
        const game = await getGame(gameId);
        ...
        const confirmed = confirm(
            (t("confirmRemoveGame") || "Remove \"{title}\" from your library?").replace("{title}", game.title || "this game")
        );
        ...
        // If currently playing this game, go back to bookshelf
        if (currentLoadedGameId === gameId)
        {
            currentLoadedGameId = null;
            currentLoadedFileData = null;
        }
    ...
    // Add game to library without launching it
    async function addGameToLibraryOnly(file)
    {
        try
        {
            const fileData = await file.arrayBuffer();
            const tempZip = await JSZip.loadAsync(fileData);

            // Locate and parse core file
            const corePath = await locateCoreFile(tempZip);
            const coreDir = dirname(corePath);

            // Read core file text directly from tempZip
            const coreFile = tempZip.file(corePath);
            if (!coreFile) throw new Error(`Missing file in ZIP: ${corePath}`);
            const coreXml = await coreFile.async("string");
            const coreDoc = parseXml(coreXml);

            // Extract metadata
            const md = coreDoc.querySelector("metadata");
            let title = "Untitled", creator = "Unknown", description = "";
            ...
            if (md)
            {
                ...
                if (creatorEl?.textContent?.trim()) creator = creatorEl.textContent.trim();
            ...
            if (manifest)
            {
                for (const el of Array.from(manifest.children).filter(x => x.tagName === "item"))
                {
                    ...
                    if (role === "egf_cover")
                    {
                        ...
                        const mediaType = el.getAttribute("media-type") || "image/jpeg";
                        const hrefPath = resolveRelative(coreDir, hrefRaw);

                        // Read cover and convert to data URL
                        const coverFile = tempZip.file(hrefPath);
                        if (coverFile)
                        {
                            const coverData = await coverFile.async("base64");
                            coverDataUrl = `data:${mediaType};base64,${coverData}`;
                        }
                        break;
                    }
                }
            ...
            const existingGame = libraryGames.find(g =>
                g.title === title && g.creator === creator
            );

            if (existingGame)
            {
                const overwrite = confirm(
                    (t("gameExistsOverwrite") || "A game with this title already exists. Overwrite?")
                );
                if (!overwrite) return;

                existingGame.fileData = fileData;
                existingGame.coverDataUrl = coverDataUrl;
                existingGame.description = description;
                existingGame.updatedAt = Date.now();

                await saveGame(existingGame);
                alert(t("gameUpdated") || "Game updated in library!");
            }
            else
            {
                const gameData = {
                ...
                alert(t("gameSaved") || "Game saved to library!");
            }
        ...
        {
            console.error("Failed to add game to library:", e);
            alert((t("error") || "Error") + ": " + (e.message || String(e)));
        }
    }
    ...
    // Cover blob URL (shared by header + About)
    ...
    function revokeSceneObjectUrls()
    {
        for (const url of sceneObjectUrls)
        {
            ...
            // Do not revoke cover here (managed by revokeCoverUrl)

            if (coverObjectUrl && url === coverObjectUrl)
            {
                continue;
            }
    ...
    // Set cover in both header and About
    ...
    function shouldAllowFgOnRole(role)
    {
        return true;
    }
    ...
    async function render_game_title_simple(sceneId, manifestItem)
    {
        ...
        if (audItem)
        {
            ...
            if (!ok)
            {
                ...
                hint.style.marginTop = "10px";
                hint.innerHTML = `<b>${escapeHtml(t("audioBlockedTitle") || "Audio blocked")}</b><br>
        <span class="muted">${escapeHtml(t("audioBlockedHint") || "Tap Start to enable audio on mobile browsers.")}</span>`;
    ...
    function validateMimetypeFirstOrThrow(arrayBuffer)
    {
        ...
        const extraLen = readU16LE(dv, off + 28);
    ...
    async function loadPackage(file)
    {
        ...
        // Revoke old cover URL and hide cover UI
        ...
        if (gameState.coverItem?.hrefPath)
        {
            try
            {
                const url = await readZipBlobUrl(
                    gameState.coverItem.hrefPath,
                    gameState.coverItem.mediaType || "image/jpeg"
                );

                // setCoverUrl handles removing it from tracking + alt + revoking the previous one

                setCoverUrl(url, meta?.title || "");
            }
    ...
    (function initLanguage()
    {
        ...
        // If the user has already chosen a language, respect it
        ...
        // Otherwise, infer it from the browser (e.g., "fr-FR", "en-US", etc.)
        // navigator.languages is more reliable than navigator.language (preference list)
        ...
        function normalizeLocaleToAppLang(locale)
        {
            ...
            // ex: "fr-FR" -> "fr", "pt-BR" -> "pt", "zh-Hans-CN" -> "zh"

            const base = raw.toLowerCase().split(/[-_]/)[0];

            // Optional tweaks (if you add variants later)
            // e.g., "he" (Hebrew) could be mapped to "ar"? (we’re not doing that here)
            // e.g., "zh" stays "zh"
        ...
        // Try an exact match (in case you later support full tags)

        for (const loc of prefs)
        {
            const exact = String(loc || "").trim();
        ...
        // Try the base language ("fr-FR" → "fr")
        ...
        if (!picked || !allowed.has(picked)) picked = "en";
    ...
    } = {})
    {
        ...
        if (libraryTitle) libraryTitle.textContent = t("myLibrary") || "📚 My Library";
        if (btnAddGame) btnAddGame.innerHTML = t("addGame") || "➕ Add Game";
        if (btnBackToLibrary) btnBackToLibrary.textContent = t("backToLibrary") || "← Library";
        if (btnSaveToLibrary) btnSaveToLibrary.textContent = t("saveToLibrary") || "💾 Save to Library";
        if (ctxPlay) ctxPlay.textContent = t("play") || "▶️ Play";
        ...
        if (!zip)
        {
            sceneName.textContent = t("noPkgTitle");
            sceneSub.textContent = t("noPkgSub");
            sceneContent.innerHTML = `<div class="textBlock muted">${escapeHtml(t("loadHint"))}</div>`;
        }
        else
        {
            updateHeaderIdentity();
        ...
        // Update bookshelf UI text
        renderBookshelf();
    }

    // =========================
    // BOOKSHELF EVENT LISTENERS
    // =========================

    // Add game buttons
    btnAddGame?.addEventListener("click", triggerFileInput);

    // Back to library button
    btnBackToLibrary?.addEventListener("click", async () =>
    {
        if (zip && gameState.sessionActive)
        {
            const ok = confirm(t("confirmBackToLibrary") || "Leave the current game and go back to library?");
            if (!ok) return;
        }
    ...
    // Context menu items
    ctxPlay?.addEventListener("click", async () =>
    {
        const gameId = currentContextGameId; // Capture before hiding
        hideContextMenu();
        if (gameId)
        {
            await playGameFromLibrary(gameId);
        }
    });
    ...
    {
        if (!e.target.closest(".context-menu") && !e.target.closest(".book-menu-btn"))
        {
            hideContextMenu();
        }
    });
    ...
    async function initBookshelf()
    {
        try
        {
            await initDatabase();
            await renderBookshelf();
            showBookshelf(); // Start on bookshelf view
        }
        catch (e)
        {
            console.error("Failed to initialize bookshelf:", e);
            // Fallback: show bookshelf anyway with empty state
            showBookshelf();
        }
    }
